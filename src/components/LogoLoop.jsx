  ...
  ({
    ...
    const renderLogoItem = useCallback(
      (item, key) => {
        if (renderItem) {
          return (
            <li
              ...
              style={isVertical ? { marginBottom: `var(--logoloop-gap)` } : { marginRight: `var(--logoloop-gap)` }}
              key={key}
              role="listitem"
            >
              {renderItem(item, key)}
        ...
        const content = isNodeItem ? (
          <span className={cx('logo-loop-inline', scaleOnHover && 'logo-loop-hover-grow')}>{item.node}</span>
        ) : (
          <img
            className={cx('logo-loop-img', scaleOnHover && 'logo-loop-hover-grow')}
            src={item.src}
            srcSet={item.srcSet}
            sizes={item.sizes}
            width={item.width}
    ...
    const logoLists = useMemo(
      () =>
        Array.from({ length: copyCount }, (_, copyIndex) => (
          <ul
            className={cx('logo-loop-seq', isVertical && 'logo-loop-seq-vertical')}
            key={`copy-${copyIndex}`}
            role="list"
    ...
    return (
      <div ref={containerRef} className={rootClasses} style={containerStyle} role="region" aria-label={ariaLabel}>
        {fadeOut && !isVertical && (
          <>
            <div aria-hidden className="logo-loop-fade logo-loop-fade-left" />
            <div aria-hidden className="logo-loop-fade logo-loop-fade-right" />
          </>
        )}

        <div
          className={cx('logo-loop-track', isVertical && 'logo-loop-track-vertical')}
          ref={trackRef}
